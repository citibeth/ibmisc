...
@param transpose Use '.' for regular, 'T' for transpose. */
Mxb<double, Eigen::ColMajor, int> VarTransformer::apply_scalars(
    ...
    for (int i=0; i < n_outputs_nu; ++i) {
        for (int j=0; j < n_inputs_wu; ++j) {
            ...
            if (j == unit_inputs) {
                ...
                if (coeff != 0) {
                    if (transpose == 'T') {
                        triplets.push_back(Eigen::Triplet<double,int>(j,i,coeff));
                    } else {
                        triplets.push_back(Eigen::Triplet<double,int>(i,j,coeff));
                    }
                }
            }
        }
...
//std::cout << "apply_scalars() returning M=" << ret.M << std::endl;
//std::cout << "apply_scalars() returning b=" << ret.b << std::endl;
